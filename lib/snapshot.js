...
function createSnapshot ({targetDayString, msgstorePath, waPath}, cb) {
  ...
  debug(`startLimit: ${startLimit}`)
  let endLimit = targetDay.endOf('day').valueOf()
  debug(`endLimit: ${endLimit}`)
    ...
    createSnapshot: (getMessages, getContacts, callback) => {
      ...
      messages.forEach(message => {
        ...
        if (groupContact && !(groupContact.jid in selectedContacts)) {
          ...
          debug('groupContact: ' + JSON.stringify(groupContact))
      ...
      // debug
      debug(`Parsed ${messages.length} messages on ${targetDayISO}:`)
      messages.forEach(message => {
        let previewText = message.type === waMessage.TEXT
          ? message.text.substring(0, 20)
          : ''
        debug(`\t[${message.receivedTimestamp} - ${message.type}] ${previewText}`)
      })
      debug(`Parsed ${selectedContactsArray.length} contacts:`)
      selectedContactsArray.forEach(contact => {
        debug(`\t[${contact.jid}] ${contact.displayName || contact.waName}`)
      })
      waMap[targetDayISO] = {
