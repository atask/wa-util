...
var chats = [],
    ...
    // more restrictive selection... add profile photo?
    //QUERY_GET_USERS = 'SELECT * FROM wa_contacts WHERE is_whatsapp_user = 1',
    QUERY_GET_USERS = 'SELECT jid, display_name, wa_name, sort_name FROM wa_contacts WHERE is_whatsapp_user = 1',
    
    //QUERY_GET_MSG = 'SELECT * FROM messages WHERE key_remote_jid=? ORDER BY _id ASC;',
    QUERY_GET_MSG = 'SELECT * FROM messages WHERE key_remote_jid=$jid ORDER BY received_timestamp ASC;'
...
function load(waPath, msgstorePath, callback) {
    ...
    // get chat ids
    msgstore.each(QUERY_GET_CHATS, [], function parseChat(msgstoreErr, chat) {
        chat.key_remote_jid
